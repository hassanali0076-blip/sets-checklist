...
The quiet local repair now passes all 14 controlled physical-iPhone checks as well as the compiled browser run and 61 sync tests. Temporary uploads retry without another action, exact failed payloads are retained, and downloads wait for acknowledged uploads. Real-service verification and integration remain pending. Nothing is released; PowerSync coverage remains separate.

Next: Prepare bounded integration and verify real service boundaries before release, preserving upload ordering and failed-payload protection.
...
The quiet local repair now passes all 14 controlled physical-iPhone checks. Rejected completion and completed-set edits recover automatically, retain exact values, save once and survive reopening. Technical prompts and manual retry instructions are removed; completion still waits for a confirmed save. The 482 logger/recovery tests also pass. Nothing is released.

Next: Prepare bounded integration, verify real-service behavior and obtain release approval without regressing existing fixes.
...
The quiet local repair passes all 14 controlled physical-iPhone and compiled browser checks. All 500 synthetic older workouts and the previous active save survive temporary full storage. Rejected sets remain incomplete until automatic retry saves once and survives reopening. Real-service verification and integration remain pending; nothing is released.

Next: Preserve the history policy during integration and address the separate database-open reset risk before release.
...
ID: sets-20260905-equipment-edit-blank-plan · Progression · Assigned to Codex · Record updated 2026-09-06
...
The quiet local repair passes all 14 controlled physical-iPhone checks and 482 logger/recovery tests. Temporary draft and recovery failures retry automatically with passive saving status, retaining exact input, safe startup navigation, fallback storage, durable cleanup and new-start guards. No technical error or Retry save prompt is required for these temporary failures. Permanent storage exhaustion and other legacy readers remain separate.

Next: Prepare bounded integration and real-service checks. Keep total-storage-failure limits explicit and complete the remaining database-open, source-deletion and damaged-data checks.
